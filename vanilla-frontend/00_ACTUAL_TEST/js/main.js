...
function transformPrev(event) {
   ...
   * 즉, liList.length * 260 + Number(activeLi) 는 현재 위치부터 오른쪽으로 나열되야 하는 나머지 카드들으 ㅣ너비
   */
  ...
  if (classList.clientWidth < liList.length * 260 + Number(activeLi)) {
     ...
     * 넘치는 li 가 없다는 뜻으로, NEXT 버튼은 활성화 되면 안됨.
     */
    if (classList.clientWidth > liList.length * 260 + Number(activeLi)) {
...
for (let i = 0; i < slidePrevList.length; i++) {
  ...
  // 카드가 ul 태그 너비보다 넘치면, 왼쪽(prev) 버튼은 활성화 되고, 오른쪽(next) 는 현재 맨 첫카드 위치이므로 비활성화
  if (classList.clientWidth < liList.length * 260) {
    ...
    1. 먼저 부모 요소를 찾고,
    2. 부모 요소의 자식 요소로 있는 PREV 와 NEXT 요소를 삭제해야 한다.
    */
